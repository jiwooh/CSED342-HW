...
def find_frequent_words(text:str, freq:int)->Set[str]:
    ...
    for i in text.split(): x[i] += 1
    return {key for key, value in x.items() if value == freq}
    # END_YOUR_ANSWER 
...
def find_nonsingleton_words(text: str) -> Set[str]:
    ...
    x = collections.defaultdict(int)
    for i in text.split(): x[i] += 1
    return {key for key, value in x.items() if value > 1}
    # END_YOUR_CODE
...
def increment_dense_vector(v1:DenseVector, scale:float, v2:DenseVector)->DenseVector:
    ...
    """
    # BEGIN_YOUR_ANSWER (our solution is 1 lines of code, but don't worry if you deviate from this)
    return [x[0] + scale * x[1] for x in zip(v1, v2)]
    # END_YOUR_ANSWER
...
def dense_to_sparse_vector(v:DenseVector)->SparseVector:
    ...
    """
    # BEGIN_YOUR_ANSWER (our solution is 1 lines of code, but don't worry if you deviate from this)
    return dict(x for x in enumerate(v) if x[1] != 0)
    # END_YOUR_ANSWER
...
def increment_sparse_vector(v1: SparseVector, scale: float, v2: SparseVector,
    ...
    for key in v2: v1[key] += scale * v2[key]
    # END_YOUR_CODE
...
def euclidean_distance(loc1: Position, loc2: Position) -> float:
    ...
    """
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    return math.sqrt(sum((x[0] - x[1]) ** 2 for x in zip(loc1, loc2)))
    # END_YOUR_CODE
...
def mutate_sentences(sentence: str) -> List[str]:
    ...
    if length <= 1: return [sentence] # edge case
    for i in range(length - 1): # create adjacency list
        if words[i] not in graph:
            graph[words[i]] = set() # new entry
        graph[words[i]].add(words[i + 1])

    def dfs(word, path, res, depth):
        if depth == length:
            res.add(' '.join(path))
            return
        for nextWord in graph.get(word, []):
            dfs(nextWord, path + [nextWord], res, depth + 1)

    for word in words: # start dfs from each word
        dfs(word, [word], res, 1)
    return list(res)
    # END_YOUR_CODE