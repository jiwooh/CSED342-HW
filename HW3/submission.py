...
class WordSegmentationProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        for i in range(state + 1, len(self.query) + 1):
            res.append((self.query[state:i], i, self.unigramCost(self.query[state:i])))
        return res
        # END_YOUR_CODE
...
class VowelInsertionProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        if index < len(self.queryWords):
            currentWordOptions = self.possibleFills(self.queryWords[index])
            if not currentWordOptions:
                currentWordOptions = {self.queryWords[index]}
            for word in currentWordOptions:
                cost = self.bigramCost(lastWord, word)
                nextState = (index + 1, word)
                res.append((word, nextState, cost))
        return res
        # END_YOUR_CODE
...
class JointSegmentationInsertionProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        for i in range(index + 1, len(self.query) + 1):
            segment = self.query[index:i]
            fills = self.possibleFills(segment)
            for fill in fills:
                cost = self.bigramCost(lastWord, fill)
                nextState = (i, fill)
                res.append((fill, nextState, cost))
        return res
        # END_YOUR_CODE
...
class MazeProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        # BEGIN_YOUR_CODE (our solution is 2 lines of code, but don't worry if you deviate from this)
        res = []
        for move, newState in self.possibleMoves(state):
            res.append((move, newState, self.moveCost(state, move)))
        return res
        # END_YOUR_CODE
